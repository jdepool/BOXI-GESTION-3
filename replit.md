...
- **Order & Payment Tracking**: Comprehensive tracking of sales, delivery statuses, multi-product orders, and reservations. Features payment verification, multi-currency support, and `Pendiente` (balance) calculation. Includes automated payment verification via bank statement reconciliation using strict confidence criteria (100% or 90% confidence for auto-verification). Internal email notifications are sent for Zelle, Binance, or Paypal payments.
