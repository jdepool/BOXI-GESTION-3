...
**File Storage**: Dispatch sheet PDFs are stored using the official `@replit/object-storage` SDK in bucket "Boxi-Guias-Transporte". The implementation uses direct server-side uploads via multer FormData processing and streaming downloads with comprehensive error handling. Content types are stored in the database and properly set on download responses. Missing files return 404 (not 500), with robust detection of various SDK error formats (status codes, error codes, message patterns). Stream errors are handled before headers are sent to prevent partial responses.
