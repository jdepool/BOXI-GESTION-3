...
- **Dynamic Cashea Pricing**: For Cashea orders ONLY, the system calculates `totalUSD` (product-level) dynamically by looking up the product name in the `productos` table to retrieve its SKU, then querying the `precios` table for the latest `precioCasheaUsd` (by `fechaVigenciaDesde`), and multiplying by `Cantidad`. The `totalOrderUsd` (order-level) remains as the "Total (USD)" value from the Cashea API. If product mapping or pricing lookup fails, the system falls back to using the Cashea API total with appropriate logging for debugging. This ensures Cashea product pricing reflects the company's internal pricing structure while maintaining order total accuracy.
