...
- **Inventory Management**: Comprehensive warehouse inventory tracking with stock levels (actual, reservado, mínimo) per product per warehouse. Features automatic stock deduction when fechaDespacho is set, handles combo product components, and allows negative stock for deficit tracking. Includes dual-mode inventory upload via single "Cargar Inventario" button: (1) Manual entry with product/warehouse selects and stock quantity inputs, (2) Excel import supporting bulk uploads with columns SKU, Almacén, Stock Actual, Stock Reservado (optional), Stock Mínimo (optional). Backend validates max 1000 records per upload, uses upsert logic (create if doesn't exist, update if exists), and returns detailed summary with created/updated/failed counts and per-row error tracking. Analytics dashboard displays stock status, alerts, and inventory value metrics.
