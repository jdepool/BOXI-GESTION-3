...
**File Storage**: Dispatch sheet PDFs are stored as Base64-encoded text directly in the PostgreSQL database (`dispatch_sheets.fileData` column). The implementation uses multer for PDF-only uploads, converts buffers to Base64 for storage, and decodes back to binary for downloads. This approach provides simplicity and reliability - files are automatically deleted when the associated order is marked "Entregado" via CASCADE delete. No separate file storage cleanup is needed. Admin dispatch sheet management in the Despachos table uses a dual-button UI: a FileText icon button for download (anchor tag with session auth) and a separate Trash icon button with AlertDialog confirmation for delete. Guest access uses separate download endpoint (`/api/guest/dispatch-sheets/:id/download`) with JWT bearer token authentication, while admin endpoint (`/api/dispatch-sheets/:id/download`) uses session-based auth to prevent authentication method conflicts.
