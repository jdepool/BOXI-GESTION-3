...
**File Storage**: Dispatch sheet PDFs are stored as Base64-encoded text directly in the PostgreSQL database (`dispatch_sheets.fileData` column). The implementation uses multer for PDF-only uploads, converts buffers to Base64 for storage, and decodes back to binary for downloads. This approach provides simplicity and reliability - files are automatically deleted when the associated order is marked "Entregado" via CASCADE delete. No separate file storage cleanup is needed. Admin dispatch sheet management in the Despachos table uses a dropdown menu with FileText icon: click to see options for "Ver/Descargar PDF" (anchor tag download) and "Eliminar" (with window.confirm confirmation). Guest users see a simple download button that fetches the same endpoint. Both download and delete endpoints (`/api/dispatch-sheets/:id/download` and `/api/dispatch-sheets/:id`) have no authentication for simplicity - both admin and guest users access the same endpoints.
...
- **Guest Access System**: JWT-based token authentication for third-party users with scoped permissions (Despacho, Inventario). Features shareable URLs, token revocation, audit trails, and real-time activity monitoring. Server-side security enforces field stripping and logs all guest actions. All guest endpoints use `validateGuestToken` + `requireGuestScope()` middleware chain to enforce proper scope checking - guest-auth utilities are hoisted to top of registerRoutes function to prevent duplicate import errors. Both middlewares check session authentication via `req.session?.user?.isAuthenticated` to allow admin access. Guest Despacho view includes full component expansion matching the admin view - orders with multiple components (e.g., combo products) display as separate rows per component with componenteSku and cantidadComponente fields. The system enriches NULL SKUs via product name matching before component expansion to ensure Cashea MP and similar orders display component details correctly. The guest view includes a "Guía de Despacho" column positioned after "Nro Guía" that displays download buttons for existing dispatch sheet PDFs. Guest users can download existing dispatch sheets but cannot upload or delete (admin-only operations). The `/api/guest/sales` endpoint joins with `dispatch_sheets` table to include `dispatchSheetId` and `dispatchSheetFileName` in the response.
