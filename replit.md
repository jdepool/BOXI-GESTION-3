...
- **Automation**: Configurable automated Cashea order downloads and payment detail assignment. **Dual Cashea Portal Support**: The automation system supports two independent Cashea portals - "Cashea" (Boxi Sleep using marketplace@boxisleep.com) and "Cashea MP" (Mompox using marketplace@mompox.com). Users can select the portal via a dedicated dropdown in the automation configuration UI. The system automatically uses the appropriate credentials based on the selected portal. All automation features (scheduled downloads, manual downloads, history tracking) respect the configured portal setting.
