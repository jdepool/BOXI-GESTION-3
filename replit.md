...
- **Precios/Costos Management**: Comprehensive pricing and cost tracking system in the Administración section (PRECIOS/COSTOS tab). Allows administrators to track product prices (Inmediata and Reserva) and unit costs in USD with effective dates (Fecha Vigencia Desde). Supports multiple pricing/cost records for the same SKU with different effective dates to track profit margins over time as prices and costs change independently. País defaults to Venezuela but is changeable. Features full CRUD operations with table display sorted by effective date (newest first).
