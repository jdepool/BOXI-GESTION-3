...
- **Prospectos (Lead Management)**: Complete lead tracking system with separate numbering (P-0001, P-0002...) from sales orders. Features include: dedicated Prospectos tab as first tab in Sales section, minimal MVP fields (nombre, telefono required; canal defaults to "Tienda", asesor optional), filtering by asesor and estado prospecto, full CRUD operations via ProspectosTable and ProspectoDialog components, pagination, and automatic prospecto number generation. Designed for early-stage customer inquiries before conversion to actual sales. **Estado Prospecto Lifecycle**: Prospectos progress through three states: 'Activo' (default, shown by default), 'Fallido' (manually marked when customer won't complete purchase, available at any phase via Seguimiento dialog), and 'Convertido' (automatically set when prospecto is converted to a sale). Fallido/Convertido prospectos are hidden from default view but accessible via Estado filter dropdown. Seguimiento status badges (follow-up phase colors) are only shown for Activo prospectos.
- **CRM Follow-up System**: Structured 3-phase follow-up workflow for Prospectos with automatic date calculations and visual status tracking. Features include: Seguimiento dialog with automatic date proposals based on configurable intervals (configured via Administración > SEGUIMIENTO tab), customizable follow-up dates with cascading logic, notes tracking for each phase (respuestaSeguimiento1/2/3), and Estado Seguimiento column showing phase number (1/2/3) with color-coded status badges (red=overdue, yellow=today, green=future with date display). The Seguimiento button is positioned between Editar and Convertir in the actions column. System uses timezone-safe date-only comparisons to ensure accurate status display across different timezones.
- **Multi-Asesor Email Configuration**: Administrators can configure up to 5 asesor-email pairs in the SEGUIMIENTO tab, enabling targeted daily reminder emails. Each asesor can have a dedicated email address with validation to ensure uniqueness and proper format. A general fallback email is required for prospectos without an assigned asesor or when the asesor doesn't have a configured email. The system includes both frontend and backend email validation.
- **Automated Daily Reminders**: Cron job runs daily at 8:00 AM to send email reminders to asesores about scheduled follow-ups. The system queries prospectos with follow-ups due that day, groups them by asesor, and sends personalized HTML emails containing prospect details, phase information, and previous responses. Prospectos without an assigned asesor receive reminders via the general fallback email.
- **Precios/Costos Management**: Comprehensive pricing and cost tracking system in the Administración section (PRECIOS/COSTOS tab). Allows administrators to track product prices (Inmediata, Reserva, and Cashea) and unit costs in USD with effective dates (Fecha Vigencia Desde). Supports multiple pricing/cost records for the same SKU with different effective dates to track profit margins over time as prices and costs change independently. País defaults to Venezuela but is changeable. Features full CRUD operations with table display sorted by effective date (newest first), icon-only action buttons (Edit/Delete) with tooltips for cleaner UI, Excel upload capability for batch imports, and undo functionality to restore previous pricing data. The Excel upload feature validates required columns (País, SKU, Precio Inmediata USD, Precio Reserva USD, Precio Cashea USD, Costo Unitario USD, Fecha Vigencia Desde), handles both Excel date serial numbers and string dates, and provides detailed validation error messages. The Precio Cashea USD column is positioned between Precio Reserva USD and Costo Unitario USD in both the table and form. The undo button persists across page refreshes when a backup exists.
- **Dynamic Cashea Pricing**: For Cashea orders ONLY, the system calculates `totalUSD` (product-level) dynamically by looking up the product name in the `productos` table to retrieve its SKU, then querying the `precios` table for the latest `precioInmediataUsd` (by `fechaVigenciaDesde`), and multiplying by `Cantidad`. The `totalOrderUsd` (order-level) remains as the "Total (USD)" value from the Cashea API. If product mapping or pricing lookup fails, the system falls back to using the Cashea API total with appropriate logging for debugging. This ensures Cashea product pricing reflects the company's internal pricing structure while maintaining order total accuracy.
- **EditSaleModal Total Order USD Calculation**: When editing sales in Lista de Ventas, the Total Order USD automatically recalculates as the sum of all product Total USD values when products are added, edited, or removed, ensuring accurate order totals.
