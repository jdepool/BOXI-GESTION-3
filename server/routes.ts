...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Zapier webhook endpoint to receive address data for Cashea orders
  app.post("/api/webhooks/zapier-address", async (req, res) => {
    try {
      console.log("📥 Received Zapier address webhook request");
      console.log("📦 Webhook payload:", JSON.stringify(req.body, null, 2));
      ...
      // Basic validation
      if (!payload || typeof payload !== 'object') {
        console.log("❌ Invalid Zapier webhook data received");
        return res.status(400).json({ error: "Invalid webhook data" });
      }

      // Extract order number (could be 'orden', 'Orden', or 'numero_orden')
      const orderNumber = payload.orden || payload.Orden || payload.numero_orden || payload.order_number;
      
      console.log(`📦 Processing Zapier address update for order: ${orderNumber}`);

      // Validate order number
      if (!orderNumber) {
        console.log("❌ No order number provided in Zapier webhook");
        return res.status(400).json({ error: "No order number provided" });
      }
      ...
      console.log(`✅ Found ${existingOrders.length} product(s) for order ${orderNumber}`);

      // Helper function to get first defined value from multiple field variations
      const getFieldValue = (...fields: any[]) => fields.find(f => f !== undefined && f !== null && f !== '');

      // Extract address data from payload - only defined values
      // Support multiple field name variations that Zapier might send
      const rawAddressData = {
        // Shipping address (Dirección de Despacho)
        direccionDespachoPais: getFieldValue(payload.pais_despacho, payload.direccion_despacho_pais, payload.pais),
        direccionDespachoEstado: getFieldValue(payload.estado_despacho, payload.direccion_despacho_estado, payload.estado),
        direccionDespachoCiudad: getFieldValue(payload.ciudad_despacho, payload.direccion_despacho_ciudad, payload.ciudad),
        direccionDespachoDireccion: getFieldValue(payload.direccion_despacho, payload.direccion, payload.address),
        direccionDespachoUrbanizacion: getFieldValue(payload.urbanizacion_despacho, payload.urbanizacion),
        direccionDespachoReferencia: getFieldValue(payload.referencia_despacho, payload.referencia, payload.indicaciones),
        
        // Billing address (Dirección de Facturación)
        direccionFacturacionPais: getFieldValue(payload.pais_facturacion, payload.direccion_facturacion_pais),
        direccionFacturacionEstado: getFieldValue(payload.estado_facturacion, payload.direccion_facturacion_estado),
        direccionFacturacionCiudad: getFieldValue(payload.ciudad_facturacion, payload.direccion_facturacion_ciudad),
        direccionFacturacionDireccion: getFieldValue(payload.direccion_facturacion, payload.billing_address),
        direccionFacturacionUrbanizacion: getFieldValue(payload.urbanizacion_facturacion),
        direccionFacturacionReferencia: getFieldValue(payload.referencia_facturacion),
        
        // Flag if addresses are the same
        direccionDespachoIgualFacturacion: getFieldValue(payload.misma_direccion, payload.same_address, payload.direccion_igual)
      };

      // Convert boolean-like strings to 'true'/'false' for the flag field
      if (rawAddressData.direccionDespachoIgualFacturacion) {
        const val = String(rawAddressData.direccionDespachoIgualFacturacion).toLowerCase();
        rawAddressData.direccionDespachoIgualFacturacion = (val === 'true' || val === '1' || val === 'yes' || val === 'si') ? 'true' : 'false';
      }

      // Filter to keep only defined values (prevents overwriting existing data with undefined)
      const addressData: Record<string, string> = {};
      for (const [key, value] of Object.entries(rawAddressData)) {
        if (value !== undefined && value !== null && value !== '') {
          addressData[key] = String(value);
        }
      }

      // Log extracted data
      console.log(`📍 Address Data Received:`);
      console.log(`   Shipping - País: ${addressData.direccionDespachoPais || '(no enviado)'}`);
      console.log(`   Shipping - Estado: ${addressData.direccionDespachoEstado || '(no enviado)'}`);
      console.log(`   Shipping - Ciudad: ${addressData.direccionDespachoCiudad || '(no enviado)'}`);
      console.log(`   Shipping - Dirección: ${addressData.direccionDespachoDireccion || '(no enviado)'}`);
      console.log(`   Shipping - Urbanización: ${addressData.direccionDespachoUrbanizacion || '(no enviado)'}`);
      console.log(`   Shipping - Referencia: ${addressData.direccionDespachoReferencia || '(no enviado)'}`);
      
      if (addressData.direccionDespachoIgualFacturacion === 'false') {
        console.log(`   Billing (separate) - País: ${addressData.direccionFacturacionPais || '(no enviado)'}`);
        console.log(`   Billing (separate) - Dirección: ${addressData.direccionFacturacionDireccion || '(no enviado)'}`);
      } else if (addressData.direccionDespachoIgualFacturacion) {
        console.log(`   🏢 Billing address same as shipping`);
      }

      const fieldsToUpdate = Object.keys(addressData).filter(k => k !== 'direccionDespachoIgualFacturacion');
      console.log(`📝 Will update ${fieldsToUpdate.length} address field(s): ${fieldsToUpdate.join(', ')}`);

      // Update all products for this order
      const updatedSales = [];
      for (const order of existingOrders) {
        // Build update data with only defined fields
        const updateData: any = { id: order.id, ...addressData };
        
        const updated = await storage.updateSale(updateData);
        updatedSales.push(updated);
        console.log(`✅ Updated product: ${order.product} (ID: ${order.id})`);
      }

      console.log(`✅ Successfully updated ${updatedSales.length} product(s) for order ${orderNumber} via Zapier webhook`);
      
      res.status(200).json({ 
        success: true, 
        message: "Address updated successfully from Zapier",
        orderNumber: orderNumber,
        productsUpdated: updatedSales.length,
        addressFieldsUpdated: Object.keys(addressData).filter(k => addressData[k as keyof typeof addressData])
      });

    } catch (error) {
      console.error("❌ Zapier address webhook error:", error);
      res.status(500).json({ 
        error: "Failed to process Zapier address webhook",
        message: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  app.get("/api/egresos", async (req, res) => {
    try {
      ...
      const egresos = await storage.getEgresos(filters);
      res.json(egresos);
    } catch (error) {
      console.error("Get egresos error:", error);
      res.status(500).json({ error: "Failed to get egresos" });
    }
  });
  ...
  app.get("/api/egresos/count", async (req, res) => {
    try {
      ...
      const count = await storage.getTotalEgresosCount(filters);
      res.json({ count });
    } catch (error) {
      console.error("Get egresos count error:", error);
      res.status(500).json({ error: "Failed to get egresos count" });
    }
  });
  ...
  app.get("/api/egresos/:id", async (req, res) => {
    try {
      ...
      console.error("Get egreso by id error:", error);
  ...
  app.post("/api/egresos/:id/autorizar", async (req, res) => {
    try {
      const { id } = req.params;
      const { accion, notas } = req.body;
      
      if (!accion || !['Aprobar', 'Rechazar'].includes(accion)) {
        return res.status(400).json({ error: "Invalid accion. Must be 'Aprobar' or 'Rechazar'" });
      }

      const egreso = await storage.autorizarEgreso(id, accion, notas);
      if (!egreso) {
        return res.status(404).json({ error: "Egreso not found" });
      }
      res.json(egreso);
    } catch (error) {
      console.error("Autorizar egreso error:", error);
      res.status(500).json({ error: "Failed to authorize egreso" });
    }
  });

  app.post("/api/egresos/:id/registrar-pago", async (req, res) => {
    try {
      const { id } = req.params;
      const { fechaPago, montoPagadoUsd, montoPagadoBs, tasaCambio, bancoId, referenciaPago, numeroFacturaPagada } = req.body;
      
      if (!fechaPago || !bancoId) {
        return res.status(400).json({ error: "fechaPago and bancoId are required" });
      }

      const egreso = await storage.registrarPagoEgreso(id, {
      ...
      console.error("Registrar pago egreso error:", error);
      res.status(500).json({ error: "Failed to register payment" });
    }
  });

  app.post("/api/egresos/:id/verificar", async (req, res) => {
    try {
      const { id } = req.params;
      const { accion, notas } = req.body;
      
      if (!accion || !['Verificar', 'Rechazar'].includes(accion)) {
        return res.status(400).json({ error: "Invalid accion. Must be 'Verificar' or 'Rechazar'" });
      }

      const egreso = await storage.verificarEgreso(id, accion, notas);
      if (!egreso) {
        return res.status(404).json({ error: "Egreso not found" });
      }
      res.json(egreso);
    } catch (error) {
      console.error("Verificar egreso error:", error);
      res.status(500).json({ error: "Failed to verify egreso" });
    }
  });
