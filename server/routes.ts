...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const excelData = salesData.map(sale => {
        ...
        return {
          ...
          'Es Obsequio': sale.esObsequio ? 'Sí' : 'No',
          'SKU': sale.sku,
    ...
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        const newSalesData = enrichedProducts.map((product: any) => ({
          ...
          estadoVerificacionInicial: existingSales[0].estadoVerificacionInicial,
          notasVerificacionInicial: existingSales[0].notasVerificacionInicial,
          pagoFleteUsd: existingSales[0].pagoFleteUsd,
          fechaFlete: existingSales[0].fechaFlete,
          bancoReceptorFlete: existingSales[0].bancoReceptorFlete,
          referenciaFlete: existingSales[0].referenciaFlete,
          montoFleteBs: existingSales[0].montoFleteBs,
          montoFleteUsd: existingSales[0].montoFleteUsd,
          estadoVerificacionFlete: existingSales[0].estadoVerificacionFlete,
          notasVerificacionFlete: existingSales[0].notasVerificacionFlete,
  ...
  app.get("/api/sales/verification-payments", async (req, res) => {
    try {
      const { startDate, endDate, bancoId, search, tipoPago, estadoVerificacion, estadoEntrega, limit, offset } = req.query;
  ...
  app.get("/api/sales/verification-payments/export", async (req, res) => {
    try {
      const { startDate, endDate, bancoId, search, tipoPago, estadoVerificacion, estadoEntrega } = req.query;
